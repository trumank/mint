...
impl Lint for AutoVerificationLint {
    ...
    fn check_mods(&mut self, lcx: &LintCtxt) -> Result<Self::Output, LintError> {
        ...
        let mut fsd_pak_file = fs::File::open(game_pak_path)?;
...
) -> Result<ModGameplayAffectingResult, LintError>
where
    F: Read + Seek,
    M: Read + Seek,
{
    ...
    let check_asset = |data: Vec<u8>| -> Result<bool, LintError> {
        debug!("check_asset");
        ...
        .skip_data(true)
        .build()
        .map_err(|e| LintError::UnrealAssetError { source: e })?;
    ...
    let mod_lowercase_path_map = mod_pak_reader
        .files()
        .into_iter()
        .map(|p| -> Result<(String, String), LintError> {
            let j = mount.join(&p);
            let new_path = j
                .strip_prefix("../../../")
                .map_err(|e| LintError::PrefixMismatch { source: e })?;
            let new_path_str = &new_path.to_string_lossy().replace('\\', "/");

            Ok((new_path_str.to_ascii_lowercase(), p))
        })
        .collect::<Result<HashMap<_, _>, LintError>>()?;
