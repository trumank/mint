                ...
                }) = info.modio_tags.as_ref()
                {
                    match approval_status {
                        ...
                        ApprovalStatus::Sandbox => {
                            mk_searchable_tag(ctx, "Sandbox", ui, Some(egui::Color32::LIGHT_YELLOW), Some("Contains significant, possibly progression breaking, changes to gameplay"));
                        }
                    }
                            ...
                            .clicked()
                        {
                            let lint_options = BTreeMap::from([
                                ...
                                (
                                    ...
                                    self.lint_options.auto_verification,
                                ),
                            ]);
                    ...
                    if let Some(report) = &self.lint_report {
                        ...
                        egui::ScrollArea::vertical()
                            ...
                            .show(ui, |ui| {
                                ...
                                if let Some(auto_verification_failed_mods) = &report.auto_verification_failed_mods {
                                    if !auto_verification_failed_mods.is_empty() {
                                        CollapsingHeader::new(
                                            RichText::new(
                                                "⚠ Mod(s) with files that cannot be auto-verified detected",
                                            )
                                            .color(AMBER),
                                        )
                                        .default_open(true)
                                        .show(ui, |ui| {
                                            auto_verification_failed_mods.iter().for_each(|(r#mod, files)| {
                                                CollapsingHeader::new(
                                                    RichText::new(format!(
                                                        "⚠ {} includes files that cannot be auto-verified",
                                                        r#mod.url
                                                    ))
                                                    .color(AMBER),
                                                )
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let extra_ui = |ui: &mut Ui, mod_data: &mut ModData| {
                let mut mod_ids = Vec::new();
                mod_data.for_each_mod(&mod_data.active_profile, |mc| {
                    let mod_id = self
                        .state
                        .store
                        .get_mod_info(&mc.spec)
                        .map(|mi| mi.resolution.url)
                        .unwrap_or_else(|| ModIdentifier(mc.spec.url.clone()));
                    mod_ids.push(mod_id);
                });

                let mut profile_is_verified = true;
                ...
                if profile_is_verified {
                    ui.add_enabled(false, egui::Button::new(RichText::new("Verified").color(egui::Color32::BLACK))
                        .fill(Color32::LIGHT_GREEN)
                        .stroke(egui::Stroke::NONE));
                } else {
                     ui.add_enabled(false, egui::Button::new(RichText::new("Not Verified").color(egui::Color32::BLACK))
                        .fill(Color32::LIGHT_RED)
                        .stroke(egui::Stroke::NONE))
                        .on_hover_text_at_pointer("The profile may contain non-veerified mods or verification status has not yet been determined.");
                }
